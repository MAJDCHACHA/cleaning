...
const register_admin = async (req, res) => {
  try {
    ...
    if (foundUser) {
      ...
      res
        .status(201)
        .json({ accessToken, email: newUser.email, id: newUser.id });
    }
  } catch (err) {
...
const login_admin = async (req, res) => {
  try {
    ...
    if (!foundUser) {
      ...
      return res
        .status(201)
        .json({ accessToken, email: foundUser.email, id: foundUser.id });
    }
  } catch (err) {
    return res.status(500).json({ message: err.message });
  }
...
export default { register_admin, login_admin, refresh_admin, logout_admin };
